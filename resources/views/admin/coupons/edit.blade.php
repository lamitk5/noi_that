...
@section('title', 'Chỉnh Sửa Mã Giảm Giá | Mộc An Admin')
@section('page_title', 'Chỉnh Sửa Mã Giảm Giá: ' . $coupon->code)
    ...
    <div class="rounded-2xl border border-ui-border bg-surface p-6 sm:p-8 shadow-xs">
        <form action="{{ route('admin.coupons.update', $coupon) }}" method="POST" class="space-y-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-5">
                ...
                <div>
                    ...
                    <input
                        ...
                        id="value"
                        required
                        value="{{ old('value', (float) $coupon->value) }}"
                        class="w-full rounded-xl border border-ui-border bg-surface px-4 py-2.5 text-sm text-heading focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/20"
                    >
                    @error('value')
                        <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                ...
                <div>
                    ...
                    <input
                        ...
                        value="{{ old('max_discount_amount', $coupon->max_discount_amount ? (float) $coupon->max_discount_amount : null) }}"
                        class="w-full rounded-xl border border-ui-border bg-surface px-4 py-2.5 text-sm text-heading focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/20"
                    >
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <div>
                    ...
                    <input
                        ...
                        id="min_order_amount"
                        value="{{ old('min_order_amount', $coupon->min_order_amount ? (float) $coupon->min_order_amount : null) }}"
                        class="w-full rounded-xl border border-ui-border bg-surface px-4 py-2.5 text-sm text-heading focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/20"
                    >
                </div>
            ...
            <div class="flex items-center gap-3">
                ...
                >
                <label for="is_active" class="text-sm font-semibold text-heading cursor-pointer">Kích hoạt mã giảm giá này</label>
            </div>
